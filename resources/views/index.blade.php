    ...
    @foreach ($slides as $slide)
        ...
        <div class="overflow-hidden position-relative h-100">
            ...
            <div class="character_markup type2">
                <p
                class="text-uppercase font-sofia mark-grey-color animate animate_fade animate_btt animate_delay-10 mb-0">{{$slide->tagline}}</p>
            </div>
            </div>
          ...
          <div class="swiper-container js-swiper-slider" data-settings='{
            ...
            <div class="swiper-wrapper">\
                ...
                <div class="swiper-slide product-card product-card_style3">
                    ...
                    <a href="{{route('shop.product.details', ['product_slug' => $sellProduct->slug])}}">
                        <img loading="lazy" src=" {{ asset('uploads/products') }}/{{$sellProduct->image}}" width="258" height="313"
                        alt="Cropped Faux leather Jacket" class="pc__img">
                    </a>
                    </div>

                    <div class="pc__info position-relative">
                    <h6 class="pc__title"><a href="{{route('shop.product.details', ['product_slug' => $sellProduct->slug])}}">{{$sellProduct->name}}</a></h6>
                    <div class="product-card__price d-flex">
                        <span class="money price text-secondary">
                        @if ($sellProduct->sale_price)
                            <s>${{$sellProduct->regular_price}}</s> ${{$sellProduct->sale_price}}
                        @else
                            ${{$sellProduct->regular_price}}
                        @endif
                        </span>
                    </div>
                    </div>
    ...
    <div class="row">
        @foreach ($featuredProducts as $featuredProduct)
            <div class="col-6 col-md-4 col-lg-3">
                ...
                <div class="pc__img-wrapper">
                    <a href="{{route('shop.product.details', ['product_slug' => $featuredProduct->slug])}}">
                    <img loading="lazy" src=" {{ asset('uploads/products') }}/{{$featuredProduct->image}}" width="330" height="400"
                        alt="{{$featuredProduct->name}}" class="pc__img">
                    </a>
                </div>

                <div class="pc__info position-relative">
                    <h6 class="pc__title"><a href="{{route('shop.product.details', ['product_slug' => $featuredProduct->slug])}}">{{$featuredProduct->name}}</a></h6>
                    <div class="product-card__price d-flex align-items-center">
                    <span class="money price text-secondary">
                    @if ($featuredProduct->sale_price)
                        <s>${{$featuredProduct->regular_price}}</s> ${{$featuredProduct->sale_price}}
                    @else
